  ...
  container_definitions = jsonencode([
    {
      ...
      # Add secrets from Secrets Manager/Parameter Store here
      secrets = [
...
resource "aws_ecs_service" "platform" {
  ...
  # Allow external changes to desired_count (for autoscaling or manual changes)
  lifecycle {
...
resource "aws_appautoscaling_target" "platform" {
  ...
  resource_id        = "service/${var.ecs_cluster_name}/${aws_ecs_service.platform.name}"
...
resource "aws_appautoscaling_policy" "cpu" {
  ...
  resource_id        = aws_appautoscaling_target.platform[0].resource_id
  scalable_dimension = aws_appautoscaling_target.platform[0].scalable_dimension
  service_namespace  = aws_appautoscaling_target.platform[0].service_namespace
